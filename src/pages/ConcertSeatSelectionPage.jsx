...
const ConcertSeatSelectionPage = () => {
  ...
  const toggleSeat = (seat) => {
    ...
    const selectedZone = zones.find(zone => zone.rows.includes(seatRow));
    const currentZoneRows = selectedZone?.rows || [];

    const selectedRows = selectedSeats.map(seat => seat.charAt(0));
    const isSameZone = selectedRows.every(row => currentZoneRows.includes(row));

    if (selectedSeats.length === 0 || isSameZone) {
      ...
      alert(`Please select seats from only one tier: ${selectedZone.label}`);
    }
  };
  ...
  return (
    <div className="concert-seat-container">
      <h2>Select Seats for {title || "Concert"}</h2>
