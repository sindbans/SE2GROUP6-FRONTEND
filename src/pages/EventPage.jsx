...
export default function EventPage() {
  ...
  useEffect(() => {
    fetch(`${API}/api/events/${eventId}`)
      ...
      .then((d) => {
        console.log("Event object received:", d.eventDetails);
  ...
  const posterTilt = useTransform(scrollYProgress, [0, 1], [0, 8]);
        ...
        <Row className="gx-5 gy-4 justify-content-center">
          ...
          <Col md={8} lg={6}>
            ...
            >
              ...
              {/* ✅ Fixed Button to Handle ConcertSchema */}
              <motion.div
              ...
              >
                <Button
                  variant="primary"
                  size="lg"
                  className="px-5 book-btn"
                  onClick={() => {
                    const isConcert = event?.type === "ConcertSchema";
                    navigate(isConcert ? "/select-seats-concert" : "/select-seats", {
                      state: {
